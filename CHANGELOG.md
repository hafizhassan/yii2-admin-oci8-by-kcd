...
- Enh: Custom side menu via `hafizhassan\AdminOci8\Module::items` (mdmunir).
...
- Chg: Can customize return of `hafizhassan\AdminOci8\components\AccessHelper::getAssignedMenu()` with provide a callback to method (mdmunir).
...
- Chg: Remove `hafizhassan\AdminOci8\components\BizRule` (mdmunir).
...
- Chg: Remove `hafizhassan\AdminOci8\Module::allowActions`. Set access control directly with `hafizhassan\AdminOci8\components\AccessControl` (mdmunir).
- Chg: Change cache strategy (mdmunir).
- Chg: `hafizhassan\AdminOci8\components\DbManager` now inherited from `yii\rbac\DbManager` (mdmunir).
